...
const getTask = async () => {
  try {
    ...
    tasks.forEach((task) => {
      ...
      container.innerHTML = `
     <button type="button" class="delete-btn" dataTaskId=${task._id}>
    ...
    <button type="button" class="update-btn" dataTaskId=${task._id}>
      ...
      const updateTaskContainer = document.getElementById(
        "updateTaskContainer"
      );
      ...
      updateBtn.addEventListener("click", async (event) => {
        ...
        document.getElementById("title").value = taskData.data.task.title;
        document.getElementById("description").value =
          taskData.data.task.description;
        document.getElementById("priority").value = taskData.data.task.priority;
        document.getElementById("dueDate").value = new Date(
          taskData.data.task.dueDate
        )
          .toISOString()
          .split("T")[0];
        document.getElementById("assignee").value = taskData.data.task.assignee;
        ...
        updateForm.onsubmit = async (e) => {
          ...
          updateTaskContainer.style.display = "none"; // Hide the form
        };
      });
