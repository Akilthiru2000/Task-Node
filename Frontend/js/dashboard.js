      ...
      row.innerHTML = `
        ...
        <td>
          <div class="action-buttons">
            <button type="button" class="delete-btn" data-task-id="${task._id}">
            ...
            <button type="button" class="update-btn" data-task-id="${task._id}">
...
const addTaskEventListeners = () => {
  ...
  updateButtons.forEach((button) => {
    button.addEventListener("click", (event) => {
      const taskId = event.target
        .closest(".update-btn") // Fixed from .delete-btn to .update-btn
        .getAttribute("data-task-id");
      updateTask(taskId);
    });
  });
...
// Fetch tasks when DOM is loaded
